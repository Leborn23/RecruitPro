...
export default function CandidateDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const fetchOneMatch = async (query: MatchQuery): Promise<MatchRow | null> => {
        const { data } = (await query.limit(1)) as { data: MatchRow[] | null };
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0 space-y-2">
              ...
              <div className="flex flex-wrap gap-2 text-xs text-[#5d7896]">
                <span className="rounded-md border border-[#d6e2f1] bg-[#f8fbff] px-2 py-1">
                ...
                <span className="inline-flex items-center gap-1 rounded-md border border-[#d6e2f1] bg-[#f8fbff] px-2 py-1">
                  ...
                  {candidate.prev_company || '公司待确认'}
                </span>
                <span className="inline-flex items-center gap-1 rounded-md border border-[#d6e2f1] bg-[#f8fbff] px-2 py-1">
                  <MapPin className="h-3.5 w-3.5" />
                  {candidate.city || '城市待确认'}
                </span>
              </div>
            </div>
          </div>
              ...
              <div className="rounded-[20px] border border-[#d6e2f1] bg-[#f8fbff] p-4">
                ...
                <div className="space-y-2">
                  {profileEducation.map((item, index) => {
                    const school = cleanText(item.school) || cleanText(item.university) || cleanText(item.institution) || '未识别学校';
